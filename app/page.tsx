  ...
  return (
    <>
      ...
      <div className="relative h-44 overflow-hidden">
        <Image src="/HAIKEI1.png" alt="" fill className="object-cover object-top opacity-25" />
      ...
      <section className="section-fade max-w-5xl mx-auto px-4 py-16">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-bold text-[#2e7d8c] uppercase tracking-widest mb-1">6 THEMES</p>
            <h2 className="text-3xl font-black text-white">市の現状：6つのテーマ</h2>
            <p className="text-lg text-[#6b7280] mt-1">公開データから見た二戸の今</p>
          </div>
          <Link href="/powers" className="flex items-center gap-1.5 text-lg text-[#4dd4e7] hover:text-white font-bold transition-colors">
          ...
          {powers.map((power) => {
            ...
            return (
              <Link key={power.id} href={`/powers/${power.slug}`}
                className="power-card-gsap dark-card rounded-2xl p-6 block border border-[#1e2a3a] hover:border-[#2e7d8c]/60 transition-colors group">
                ...
                {power.score !== undefined && (
                  <div className="mb-4">
                    <div className="flex justify-between text-sm text-[#6b7280] mb-1">
                      <span>総合スコア</span><span className="font-bold text-white">{power.score}/100</span>
                    </div>
                ...
                <p className="text-base font-bold text-white mb-2 leading-snug">{power.headline}</p>
                <p className="text-base text-[#9ca3af] leading-relaxed line-clamp-3 mb-4">{power.summary}</p>
      ...
      {/* HAIKEI2 */}
      <div className="relative h-44 overflow-hidden">
        <Image src="/HAIKEI2.png" alt="" fill className="object-cover object-center opacity-25" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0a0e1a] via-transparent to-[#0a0e1a]" />
      </div>

      {/* Section 3: 主要指標カルーセル（左→右） */}
      <section className="section-fade py-16 bg-[#050810]">
        <div className="max-w-5xl mx-auto px-4 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-[#2e7d8c] uppercase tracking-widest mb-1">KEY INDICATORS</p>
              <h2 className="text-3xl font-black text-white">主要指標</h2>
              <p className="text-lg text-[#6b7280] mt-1">数値で見る二戸の現在地</p>
            </div>
            <Link href="/powers" className="flex items-center gap-1.5 text-lg text-[#4dd4e7] hover:text-white font-bold transition-colors">
              全指標を見る <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          <p className="text-base text-[#9ca3af] mt-3 max-w-2xl">公開統計をもとに整理した主要指標です。左から右へ流れています（ホバーで停止）。</p>
        </div>
        <IndicatorCarousel items={kpiItems} />
        <p className="text-sm text-[#4b5563] max-w-5xl mx-auto px-4 mt-3">※ 最新の数値は各出典元でご確認ください。</p>
      </section>
      ...
      <section className="section-fade py-16">
        <div className="max-w-5xl mx-auto px-4 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-[#2e7d8c] uppercase tracking-widest mb-1">COUNCIL</p>
              <h2 className="text-3xl font-black text-white">最近の議会</h2>
              <p className="text-lg text-[#6b7280] mt-1">市議会の最新セッション</p>
            </div>
            <a href="https://www.city.ninohe.lg.jp/menu/21" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-lg text-[#4dd4e7] hover:text-white font-bold transition-colors">
              議会公式 <ExternalLink className="w-4 h-4" />
            </a>
          </div>
          <p className="text-base text-[#9ca3af] mt-3 max-w-2xl">二戸市議会の定例会・臨時会の開催情報です。右から左へ流れています（ホバーで停止）。</p>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {recentMovements.slice(0, 2).map((movement) => {
              ...
              return (
                <div key={movement.id} className="dark-card rounded-2xl p-5 neon-border">
                  <div className="flex items-center gap-2 mb-2">
                    <Calendar className="w-4 h-4 text-[#2e7d8c]" />
                    <span className="text-sm text-[#6b7280]">{dateStr}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs bg-[#2e7d8c]/20 text-[#4dd4e7] border border-[#2e7d8c]/30">最新</span>
                  </div>
                  <p className="text-lg font-bold text-white leading-snug mb-1">{movement.title}</p>
                  <p className="text-base text-[#9ca3af] leading-relaxed line-clamp-2">{movement.summary}</p>
                </div>
              );
            })}
          </div>
          <div className="mt-4 text-right">
            <Link href="/movement" className="text-lg text-[#4dd4e7] hover:text-white font-bold transition-colors">すべての動きを見る →</Link>
          </div>
        </div>
      </section>

      {/* Section 5: キーワードマップ */}
      <section className="section-fade kw-pulse-section py-16 bg-[#050810]">
        <div className="relative z-10 max-w-5xl mx-auto px-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-xs font-bold text-[#2e7d8c] uppercase tracking-widest mb-1">KEYWORD MAP</p>
              <h2 className="text-3xl font-black text-white">キーワードマップ</h2>
              <p className="text-lg text-[#6b7280] mt-1">二戸をひもとく言葉のネットワーク</p>
            </div>
            <Link href="/graph" className="flex items-center gap-2 bg-[#2e7d8c] hover:bg-[#3a9aad] text-white font-bold px-5 py-2.5 rounded-xl transition-all text-lg shadow-lg shadow-[#2e7d8c]/30">
              <Network className="w-5 h-5" /> 全画面で開く
            </Link>
          </div>
          <p className="text-lg text-[#9ca3af] leading-relaxed mb-6 max-w-2xl">
          ...
          <div className="mt-5 text-center">
            <Link href="/graph" className="inline-flex items-center gap-2 text-xl text-[#4dd4e7] hover:text-white font-bold transition-colors">
      ...
      <section className="section-fade py-14">
        <div className="max-w-5xl mx-auto px-4">
          <div className="dark-card rounded-2xl p-8 neon-border">
            <h2 className="text-2xl font-black text-white mb-4">ニノヘミライとは</h2>
            ...
            <div className="flex flex-wrap gap-5">
              <Link href="/methodology" className="text-lg text-[#4dd4e7] hover:text-white font-semibold transition-colors">調査方法・データ出典 →</Link>
              <Link href="/about" className="text-lg text-[#4dd4e7] hover:text-white font-semibold transition-colors">運営について →</Link>
            </div>
          </div>
        </div>
      </section>
