...
export default function PowerCard({ power, indicatorCount }: PowerCardProps) {
  ...
  return (
    <Link
      ...
      className={`group block bg-white rounded-xl border border-[#e5e1da] ${colors.leftBorder} p-5 hover:shadow-md transition-all`}
    >
      ...
      <p className="text-xs text-[#6b7280] mb-3">{power.description}</p>
      ...
      <p className="text-xs text-[#6b7280] leading-relaxed line-clamp-3 mb-4">{power.summary}</p>

      {indicatorCount !== undefined && (
        <p className="text-[11px] text-[#6b7280] mb-2">指標 {indicatorCount}件</p>
      )}
